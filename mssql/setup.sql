-- Reset và setup lại toàn bộ 3 database với GAPS (SBD bị trống) để test trường hợp không tìm thấy thí sinh
-- ==========================================================
...
WHILE @i <= 300
BEGIN
    -- Bỏ qua SBD 10, 50, 100 để test trường hợp không có dữ liệu
    IF @i NOT IN (10, 50, 100)
    BEGIN
...
WHILE @k <= 600
BEGIN
    -- Bỏ qua SBD 350, 450, 550
    IF @k NOT IN (350, 450, 550)
    BEGIN
        INSERT INTO ThiSinh VALUES (
            @k, 
            CONCAT(
...
WHILE @j <= 1000
BEGIN
    -- Bỏ qua SBD 650, 750, 850
    IF @j NOT IN (650, 750, 850)
    BEGIN
        INSERT INTO ThiSinh VALUES (
            @j, 
            CONCAT(
